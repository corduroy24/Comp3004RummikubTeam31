...
public class HumanPlayerStrategyTest extends TestCase{
	...
	/*public void testPlaytiles() {
		...
		for(int i = 0; i < initialSizeOfHand; i++) {
			human.addTileToHand(deck.Draw()); 
		}
		

		double randomD = Math.random()*(14 - 0); 
		int randomI = (int)randomD; 
		Tile tileToPlay = human.getPlayerHand().get(randomI);
		//System.out.println(tileToPlay.getNumber()); 
